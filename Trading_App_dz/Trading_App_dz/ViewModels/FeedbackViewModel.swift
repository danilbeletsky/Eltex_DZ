...
final class FeedbackViewModel: ObservableObject {
    ...
    enum SwipeDirection: CaseIterable, Equatable {
        case topToBottom
        case bottomToTop
        case leftToRight
        case rightToLeft
        ...
        func matches(translation: CGSize, minimumDistance: CGFloat = 50) -> Bool {
    ...
    enum ActiveAlert: Identifiable {
        ...
        var id: String {
            switch self {
            case .submissionSucceeded: return "success"
            case .botCheckFailed: return "failure"
            }
        }
    }
    ...
    @Published var showBotCheck = false
    @Published var swipePadHighlighted = false
    ...
    @Published private(set) var currentCommandIndex = 0

    private var challengeDirections: [SwipeDirection] = []
    ...
    var currentCommandDirection: SwipeDirection? {
        guard showBotCheck, currentCommandIndex < challengeDirections.count else { return nil }
        return challengeDirections[currentCommandIndex]
    }
    ...
    func handleSwipeEnded(translation: CGSize) {
        guard showBotCheck, let expected = currentCommandDirection else { return }

        if expected.matches(translation: translation) {
            if currentCommandIndex + 1 < challengeDirections.count {
                currentCommandIndex += 1
            } else {
                completeBotCheckSuccessfully()
            }
        } else {
            failBotCheck()
        }
    ...
    func dismissAlert() {
        ...
        if alert == .submissionSucceeded {
            onSubmitted?()
        }
    }

    private func startBotCheck() {
        challengeDirections = (0..<Constants.botCheckCommandsCount).compactMap { _ in
            SwipeDirection.allCases.randomElement()
        }
        currentCommandIndex = 0
        swipePadHighlighted = false
        showBotCheck = true
    }

    private func completeBotCheckSuccessfully() {
        showBotCheck = false
        clearForm()
        activeAlert = .submissionSucceeded
    }

    private func failBotCheck() {
        showBotCheck = false
        swipePadHighlighted = false
